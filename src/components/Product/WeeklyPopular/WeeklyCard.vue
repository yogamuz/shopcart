...
<template>
  ...
  >
    ...
    <div class="p-4 flex flex-col flex-grow">
      <div class="flex justify-between items-start mb-3 min-h-[3rem]">
        <h3
          class="font-semibold text-gray-800 text-base flex-1 pr-3 line-clamp-2"
        >
          {{ weeklyProduct.title }}
        </h3>
        <span class="font-bold text-gray-900 text-base whitespace-nowrap ml-3">
          Rp {{ weeklyProduct.price.toLocaleString("id-ID") }}
        </span>
      </div>
      <p class="text-gray-600 text-sm mb-4 line-clamp-2 flex-grow-0">
        ...
        <div class="flex items-center space-x-2">
          ...
          <div class="flex items-center space-x-1">
            ...
            <span class="text-gray-500 text-sm">{{
              weeklyProduct.reviews
            }}</span>
          </div>
        </div>
      </div>

      <!-- Enhanced Add to Cart Button with Cart State Management -->
      <div class="flex justify-start">
        <button
          @click="handleAddToCart"
          :disabled="isCartLoading || isProductInCart"
          :class="[
            'interactive-element px-3 py-2 text-sm font-medium rounded-full border transition-all duration-300',
            isProductInCart
              ? 'cursor-default'
              : isCartLoading
...
const isProductInCart = computed(() => {
  ...
  return props.cartItems.some(item => item.id === props.weeklyProduct.id);
});

// Enhanced add to cart handler
const handleAddToCart = async () => {
  if (!props.weeklyProduct || !props.weeklyProduct.id) {
  ...
  try {
    ...
    emit('add-to-cart', props.weeklyProduct);
