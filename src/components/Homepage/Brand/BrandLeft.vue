<template>
  <div
    class="sliding-container overflow-hidden mb-4 sm:mb-6 lg:mb-8"
    @mouseenter="pause"
    @mouseleave="resume"
  >
    <div
      class="sliding-wrapper flex gap-3 sm:gap-4 lg:gap-6"
      :style="{ transform: `translateX(${position}px)` }"
    >
      <template v-for="(set, setIndex) in 2" :key="`left-set-${setIndex}`">
        <div
          v-for="brand in brands"
          :key="`left-${setIndex}-${brand.id}`"
...
const animate = () => {
  ...
  if (Math.abs(position.value) >= setWidth.value) {
    position.value = 0;
  }
