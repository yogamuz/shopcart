    ...
    <div class="bg-white rounded-lg lg:rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-gray-600 mt-1">{{ dateRange.startDate }} - {{ dateRange.endDate }}</p>
        </div>
    ...
    <template v-else-if="analyticsData">
        ...
        <div class="bg-white rounded-lg lg:rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl lg:text-3xl font-bold text-gray-900">
                {{ analyticsData.summary?.totalProducts || 0 }}
              </p>
            </div>
          ...
          <div class="mt-4 flex items-center gap-2">
            <span class="text-xs px-2 py-1 bg-green-100 text-green-700 rounded-full font-medium">
              {{ analyticsData.summary?.activeProducts || 0 }} Active
            </span>
            <span class="text-xs px-2 py-1 bg-gray-100 text-gray-700 rounded-full font-medium">
              {{ analyticsData.summary?.inactiveProducts || 0 }} Inactive
            </span>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg lg:rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl lg:text-3xl font-bold text-gray-900">
                {{ formatCurrency(analyticsData.pricing?.averagePrice || 0) }}
              </p>
            </div>
          ...
          <div class="mt-4">
            <p class="text-xs text-gray-600">
              Range: {{ formatCurrency(analyticsData.pricing?.minPrice || 0) }} -
              {{ formatCurrency(analyticsData.pricing?.maxPrice || 0) }}
            </p>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg lg:rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl lg:text-3xl font-bold text-gray-900">
                {{ formatCurrency(analyticsData.pricing?.totalInventoryValue || 0) }}
              </p>
            </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
        ...
        <div class="bg-white rounded-lg lg:rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6">
          ...
          <div class="h-64">
            <Line v-if="analyticsData.trends?.monthly" :data="lineChartData" :options="lineChartOptions" />
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg lg:rounded-xl shadow-sm border border-gray-200 p-4 lg:p-6">
          ...
          <div class="h-64">
            <Bar v-if="analyticsData.categories" :data="barChartData" :options="barChartOptions" />
          </div>
        </div>
      </div>
          ...
          <table class="w-full">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Products</th>
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Active</th>
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="category in analyticsData.categories" :key="category.id" class="hover:bg-gray-50">
...
const { productAnalytics, isLoading, error } = storeToRefs(productStore);

// ✅ FIX: Get action directly from store
const { fetchProductAnalytics } = productStore;
...
// ✅ Computed - use productAnalytics ref directly
const analyticsData = computed(() => productAnalytics.value);

const dateRange = computed(() => {
  if (!analyticsData.value?.dateRange) {
  ...
  return {
    startDate: formatDate(analyticsData.value.dateRange.startDate),
    endDate: formatDate(analyticsData.value.dateRange.endDate),
  };
});

// Chart.js data configuration
const lineChartData = computed(() => {
  if (!analyticsData.value?.trends?.monthly) {
    return { labels: [], datasets: [] };
  }

  const trends = analyticsData.value.trends.monthly;
  return {
